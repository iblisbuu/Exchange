...
<div class="balance-title">
    ...
    <p><?= lang('*매수 평균가, 매수 금액, 평가 금액, 평가 수익률은 모두 USDT로 환산한 추정값으로 참고용입니다.', '*The average purchase price, purchase amount, valuation amount, and rate of return are all for reference as estimates converted to USDT.', '*枚数平均原価、枚数量は、評価額、評価利回りは全てUSDTに換算した推定値に参考用です。', '* 买入均价,买入金额,估值金额,估值收益率均以USDT换算的估值作为参考。') ?></p>
</div>
<div class="balance-content">
    <table class="balance-table asset-table">
        <colgroup>
            <col width="32%">
            <col width="17%">
            <col width="17%">
        ...
        <thead>
        <tr>
            <th class="text-left"><?= lang('총 자산', 'Total Asset Valuation', '総資産', '总资产') ?></th>
            <th><?= lang('총 보유 USDT', 'USDT Current Balance', '総保有USDT', '持有USDT') ?></th>
            <th><?= lang('총 매수 금액', 'Total Purchase Amount', '総枚数金額', '收购金额') ?></th>
            <th><?= lang('총 평가 금액', 'Total Valuation', '評価額', '估值金额') ?></th>
            <th class="double-td">
                <span><?= lang('총 평가 손익률', 'Total Return on Investment', '評価損益率', '评估损益率') ?></span>
                <span><?= lang('총 평가 손익', 'Total Profit Valuation', '評価損益', '评估损益') ?></span>
            </th>
        </tr>
        </thead>
        <tbody>
        <tr>
            <td class="text-left all-asset"><?= round_down_format_fix($member['mb_usdt'] + $totalCoinsUsdtPrice, 8) ?></td>
            <td><?= round_down_format_fix($member['mb_usdt'], 8) ?></td>
            <td><?= round_down_format_fix($totalCoinsBuyPrice, 8) ?></td>
            <td><?= round_down_format_fix($totalCoinsUsdtPrice, 8) ?></td>
            <td class="double-td">
                <span class="color-balance-<?= $totalCoinsUsdtPrice > $totalCoinsBuyPrice ? 'red' : ($totalCoinsUsdtPrice < $totalCoinsBuyPrice ? 'blue' : '') ?>"><?= round_down( ($totalCoinsUsdtPrice - $totalCoinsBuyPrice) / ($totalCoinsBuyPrice) * 100, 2) ?>%</span>
                <span class="color-balance-<?= $totalCoinsUsdtPrice > $totalCoinsBuyPrice ? 'red' : ($totalCoinsResultPrice < $totalCoinsBuyPrice ? 'blue' : '') ?>"><?= round_down_format_fix($totalCoinsUsdtPrice - $totalCoinsBuyPrice, 8) ?></span>
            </td>
        </tr>
        </tbody>
    </table>
</div>
<div class="balance-title margin-top">
...
<div class="balance-content">
    <table class="balance-table retain-table">
        ...
        <tr>
            ...
            <th>
                <button onclick="tableSort(this,'buy',true,1)" class="double-td">
                    <span><?= lang('매수 금액', 'Purchase Amount', '買収金額', '收购金额') ?> (USDT)</span>
            ...
            <th>
                <button onclick="tableSort(this,'profit',true,1)" class="double-td">
                    <span><?= lang('평가 손익률', 'Return on Investment', '評価損益率', '评估损益率') ?> (%)</span>
        ...
        foreach ($list as $row) {
            ...
            <tr>
                <td class="coin-td">
                    <img src="/public/img/coin/<?= $row['ci_symbol'] ?>.png"/>
                    <div>
                ...
                <td class="double-td buy-td" data-type="buy">
                    <span><?= round_down_format_fix($row['totalCoinsResult'] + $row['totalDepositResult'], 8) ?></span>
